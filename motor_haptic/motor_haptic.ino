...
//  DEBUG_SERIAL.begin(115200);
  ...
  //dxl.writeControlTableItem(POSITION_P_GAIN, ID_2, position_p_gain);
  //dxl.writeControlTableItem(POSITION_P_GAIN, ID_2, position_i_gain);
  //dxl.writeControlTableItem(POSITION_P_GAIN, ID_2, position_d_gain);
...
void loop() {
  // robot following controller
  if (dxl.getPresentVelocity(ID_1)){
    dxl.torqueOn(ID_2);
    pos_flw(2);
    //pos_sim();  // use this function replacing "delay"
  ...
  // controller following robot
  else if (dxl.getPresentVelocity(ID_2)){
    dxl.torqueOn(ID_1);
    pos_flw(1);
    //pos_sim();   
    dxl.torqueOff(ID_1);
...
void pos_sim(){
  while(dxl.getPresentPosition(ID_1) == dxl.getPresentPosition(ID_2));
}
...
void pos_flw(int a){
  if (a == 2){
    ...
    vel_2 = dxl.getPresentVelocity(ID_2);
    pos = dxl.getPresentPosition(ID_2, UNIT_DEGREE);
    dxl.setGoalPosition(ID_1, pos, UNIT_DEGREE);
  }
}
