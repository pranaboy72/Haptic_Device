...
void loop() {
  ...
  // stop1: 로봇 앞 조인트, stop2: 로봇 중간 조인
  if ((dxl.getPresentVelocity(ID_2) == 0 ) && (vel_1 * dxl.getPresentVelocity(ID_1) > 0 || vel_0 * dxl.getPresentVelocity(ID_4))) stop1 = 1;
  if ((dxl.getPresentVelocity(ID_2) == 0 ) && (vel_1 * dxl.getPresentVelocity(ID_1) > 0 || vel_0 * dxl.getPresentVelocity(ID_4))) stop2 = 1;
  ...
  if (stop1 || stop2){
    while(1){
      ...
      if (-vel_1 * dxl.getPresentVelocity(ID_1) || -vel_0 * dxl.getPresentVelocity(ID_4)){
        if ((dxl.getPresentVelocity(ID_1)-dxl.getPresentVelocity(ID_4))<0.5){
          dxl.torqueOff(ID_1);
          dxl.torqueOff(ID_4);
          stop1 = 0;
          continue;
        ...
        if(dxl.getPresentVelocity(ID_1) == 0) dxl.torqueOff(ID_1);
  
        if(-vel_1 * dxl.getPresentVelocity(ID_1)){
          dxl.torqueOff(ID_1);
          stop2 = 0;
          continue;
        }
